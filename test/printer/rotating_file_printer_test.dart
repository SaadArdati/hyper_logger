...
void main() {
  ...
  group('size-based rotation', () {
    test('rotates when bytes written exceed maxBytes', () async {
      ...
      final printer = RotatingFilePrinter(
        baseFilePathProvider: () => path,
        rotations: [FileRotation.size(100)],
    ...
    test('respects maxFiles by deleting the oldest rotated files', () async {
      ...
      final printer = RotatingFilePrinter(
        baseFilePathProvider: () => path,
        rotations: [FileRotation.size(30)],
    ...
    test(
      'compress=true gzips rotated files and removes the .log original',
      () async {
        final path = '${tempDir.path}/app.log';
        final printer = RotatingFilePrinter(
          baseFilePathProvider: () => path,
  ...
  group('rotation: filename and retention', () {
    test(
      'rotated filenames embed a compact UTC timestamp with a Z suffix',
      () async {
        final path = '${tempDir.path}/app.log';
        final printer = RotatingFilePrinter(
          baseFilePathProvider: () => path,
        ...
        expect(
          name,
          matches(RegExp(r'^app\.\d{8}T\d{6}Z(\.\d+)?\.log$')),
          reason:
              'rotated name "$name" must include a "Z" suffix and 8+6 digits',
        );
      },
    );

    test('_enforceMaxFiles counts a (.log + .log.gz) pair as ONE rotation '
        '(round-10b dedup fix)', () async {
      final path = '${tempDir.path}/app.log';
      ...
      expect(
        ...
        2,
        reason:
            'expected 2 unique rotation timestamps after enforcement, '
      ...
      expect(
        ...
        reason:
            'a kept rotation must keep BOTH halves of its '
    ...
    test(
      ...
      () async {
        ...
        final printer = RotatingFilePrinter(
          baseFilePathProvider: () => path,
          rotations: [FileRotation.size(30)],
          retention: FileRetention(maxFiles: 1),
          formatter: (e) => 'y' * 40,
        );
        await printer.ready;
        printer.log(_entry('one'));
    ...
    test('compressed rotation does not overwrite a previous .gz with the same '
        'second-precision timestamp (verified by content)', () async {
      final path = '${tempDir.path}/app.log';
      var counter = 0;
      final printer = RotatingFilePrinter(
        baseFilePathProvider: () => path,
    ...
    test('drop-notice synthetic record under tiny maxBytes triggers rotation '
        'cleanly without losing data', () async {
      final path = '${tempDir.path}/app.log';
      final printer = RotatingFilePrinter(
        baseFilePathProvider: () async {
        ...
        rotations: [FileRotation.size(50)],
        pendingBufferSize: 3,
      );
    ...
    test(
      'time-based rotation window inherits file mtime across restarts',
      () async {
        final path = '${tempDir.path}/app.log';
  ...
  group('error handler (onError)', () {
    test(
      'init failure invokes onError with the underlying exception',
      () async {
        Object? captured;
        final printer = RotatingFilePrinter(
          baseFilePathProvider: () async {
    ...
    test('an async onError that rejects does not crash the printer', () async {
    ...
    test(
      'reentrant onError that logs through HyperLogger does not loop',
      () async {
    ...
    test('async onError coalesces failures during the handler\'s awaited '
        'tail (intentional first-error-wins window)', () async {
      ...
      expect(
        ...
        equals(baseline),
        reason:
            'failures during the handler\'s awaited tail must '
    ...
    test('drain aggregate StateError surfaces AFTER the async handler '
        "Future settles — round-9 fix for round-8's regression", () async {
      ...
      expect(
        ...
        equals(1),
        reason:
            'first per-entry error fires; the rest are '
      ...
      expect(
        ...
        isNotEmpty,
        reason:
            'aggregate StateError must fire after the async '
    ...
    test('sync reentrancy from inside the handler is still blocked '
        '(stack-overflow protection preserved)', () async {
      ...
      expect(
        ...
        reason:
            'sync reentrancy guard must keep handler invocations '
    ...
    test('async same-printer reentrant onError: handler routes through '
        'HyperLogger whose root IS this printer; sustained handle loss '
        'must not microtask-livelock', () async {
      ...
      expect(
        ...
        lessThan(20),
        reason:
            'guard must coalesce post-await reentries from inside '
  ...
  group('concurrent lifecycle', () {
    ...
    test(
      'concurrent close() callers receive the same Future identity',
      () async {
    ...
    test(
      ...
      () async {
        ...
        expect(
          all,
          contains('5 buffered entries'),
          reason:
              'init-failure must surface the count of dropped pending '
              'entries, not silently swallow them',
        );
      },
    );

    test('init-failure drop count includes entries FIFO-evicted before the '
        'path resolved', () async {
      ...
      expect(
        all,
        contains('10 buffered entries'),
        reason:
            'init-failure drop count must include entries already '
            'FIFO-evicted from the pending buffer, not just the '
            'survivors',
      );
    });

    test('log() calls after rotation handle loss surface via onError '
        '(no silent durability loss)', () async {
      ...
      expect(
        ...
        reason:
            'each failed log after handle loss must surface via '
    ...
    test('close() drains pending entries that trigger rotation and still '
        'closes the swapped-in handle (no leak)', () async {
      ...
      final printer = RotatingFilePrinter(
        ...
        rotations: [FileRotation.size(200)],
    ...
    test(
      ...
      () async {
        ...
        expect(
          errorCount,
          greaterThan(preFailureCount),
          reason:
              'rotation reopen must surface via onError once the '
  ...
  group('lifecycle', () {
    ...
    test(
      'log() after init failure becomes a true no-op (no buffer churn)',
      () async {
        final printer = RotatingFilePrinter(
          baseFilePathProvider: () async {
            throw StateError('boom');
    ...
    test(
      'close() before async path resolves still flushes pending entries',
      () async {
        final path = '${tempDir.path}/app.log';
        final printer = RotatingFilePrinter(
          baseFilePathProvider: () async {
  ...
  group('default error handler', () {
    test('formatDefaultFileWriterError emits a leading prefix line', () {
      final out = formatDefaultFileWriterError(StateError('boom'), null);
  ...
  group('default formatter', () {
    ...
    test('non-JSON-serializable context values fall back to toString without '
        'throwing', () {
      final entry = LogEntry(
        level: LogLevel.info,
        message: 'work',
